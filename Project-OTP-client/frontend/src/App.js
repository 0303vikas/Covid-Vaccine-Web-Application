...
export default class App extends React.Component {
    ...
    _resetform = async() => {
        for(var i = 0; i <= 5; i++){
        document.getElementById('otp-input-' + i).value = null;
    }
}
                ...
                <Paper elevation={4} style={{ padding: 20, width: 300, marginBottom: 60}}>
                    ...
                    <div>
                        ...
                        {this.state.otpShow ? <div style={{width: '100%', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 5}}>
                            Didn't receive an OTP? <Button onClick={() => {this._getCode(); this._resetform()}} color="primary" style={{textTransform: 'none', fontSize: 15}}>Resend OTP</Button>
                        </div> : null }
